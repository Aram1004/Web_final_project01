...
router.get('/booking/:id',needAuth, catchErrors(async (req, res, next) => {
	const travel = await Travel.findById(req.params.id);
	const user = await User.findOne({email: req.user.email});

	let b=false;
	for(k of user.travel){
		if(k._id.equals(travel._id)){
			b=true
			break;
		}
	}
	if(b){
		req.flash('danger', '이미 예약된 상품입니다.');
	}
	else{
		req.flash('success', 'Booking Successfully.');
		user.travel.push(travel)
		user.save()
	}
