...
router.post('/register_com', async (req, res) => {
...
router.get('/login_com', async (req, res) => {
...
router.post('/admin_set', async (req, res) => {
    console.log('들어오긴 했어!')
...
router.get('/register_tour', (req, res) => {
    if (req.session.user && req.session.user.g != 0) {
        res.render('register_tour')
    } else res.send("관리자만 볼 수 있습니다!")
})
router.post('/register_tour_com', async (req, res) => {
    v = {
        id: req.body.id,
        name: req.body.name,
...
router.get('/tour_set', async (req, res) => {
    if (req.session.user && req.session.user.g != 0) {
        let v = {list: []}
...
router.get('/tour_list', async (req, res) => {
    if (req.session && req.session.user && req.session.user.id) {
        let v = {list: []}
        let r = await tour.find().toArray()
        v.list.push(r)
...
router.post('/tour_reservation', (req, res) => {
    let j = {
        id: req.body.id,
        name: req.body.name,
        price: req.body.price,
...
router.post('/tour_reservation_com', async(req, res) => {
    let j = {
        name: req.body.id,
        p: req.body.p,
        date: req.body.date,
...
router.get('/tour_reservation_view', async(req, res) => {
